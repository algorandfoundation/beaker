...
BoxName = pt.Bytes(_box_name)
BoxSize = pt.Int(_box_size)
MaxInts = pt.Int(_max_ints)
...
def add_int(
    ...
    return pt.Seq(
        array_contents := pt.App.box_get(BoxName),
        ...
        (idx := pt.ScratchVar()).store(
            pt.If(
                sorted_ints_app.state.elements == pt.Int(0),
                pt.Int(0),
                binary_search(
                    val.get(),
                    array_contents.value(),
                    pt.Int(0),
                    sorted_ints_app.state.elements - pt.Int(1),
                )
                * pt.Int(8),
            )
        ),
        ...
        output.decode(
            ...
            pt.Concat(
                ...
                pt.App.box_extract(BoxName, pt.Int(0), pt.Int(8) * pt.Int(10)),
            )
        ),
    )
...
def insert_element(buff: pt.Expr, new_val: pt.Expr, pos: pt.Expr) -> pt.Expr:
    return pt.Concat(
        ...
        pt.Extract(buff, pos, (BoxSize - pos) - pt.Int(8)),
    )
...
def box_create_test() -> pt.Expr:
    return pt.Seq(
        pt.Assert(pt.App.box_create(BoxName, BoxSize)),
        sorted_ints_app.state.elements.set(pt.Int(0)),
    )
