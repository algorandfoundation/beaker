...
Here we add a method handler for an abi method ``add`` to our application in two ways.

If the blueprint only accepts an ``Application`` and returns ``None``, we can pass it directly in the initialization of our Application

.. literalinclude:: ../../examples/blueprint/main.py
    :lines: 17-17

If the blueprint requires some arguments, we can use the ``include`` method to add it by passing the additional arguments the blueprint expects
