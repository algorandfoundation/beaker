...
class ARC18(Application):
    ...
    offers: Final[DynamicAccountStateValue] = DynamicAccountStateValue(
        stack_type=TealType.bytes, max_keys=14
    )
    ...
    def transfer_algo_payment(
        ...
        valid_transfer_group = Seq(
            ...
            (offer := ScratchVar()).store(
                self.offers[Itob(royalty_asset.asset_id())][owner.address()]
            ),
    ...
    def transfer_asset_payment(
        ...
        valid_transfer_group = Seq(
            ...
            (offer := ScratchVar()).store(
                self.offers[Itob(royalty_asset.asset_id())][owner.address()].get_must()
            ),
    ...
    def get_offer(
        ...
        return output.decode(ARC18.offers[Itob(royalty_asset.get())][owner.address()])
    ...
    def do_update_offered(acct, asset, auth, amt, prev_auth, prev_amt):
        offer_state = ARC18.offers[Itob(asset)]
        return Seq(
