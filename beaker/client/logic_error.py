...
class LogicException(Exception):
    ...
    def __str__(self):
        txid, pc = parse_logic_error(self.logic_error_str)
        line_no = self.approval_map.get_line_for_pc(pc)
        trace = tiny_trace(self.app.approval_program, line_no, 5)
        return f"Txn({txid}) had logic error at pc {pc} and source line {line_no}: \n\t{trace}"
